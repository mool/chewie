...
send_meme = (message, location, response_handler)->
  ...
  message.http( url ).get() (error, response, body)->
    ...
    img_src = get_meme_image( body, '.c1 img' )
    img_title = get_meme_title( body, 'h3 a' )
...
get_meme_image = (body, selectors)->
  for selector in selectors.split(',')
    if selected = select_element(body, selector)
      return selected.attribs.src
